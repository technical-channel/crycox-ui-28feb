...
import { ReleaseFund } from "../../utils/contractHelper";
...
import { createTradeAPI } from "../../GraphQL/Mutations/createTradeAPI";
...
const ClientBuyerPage = () => {
     const navigate = useNavigate();
     const { library, account, active, chainId } = useWeb3React();
     const { offerId, buyerId } = useParams();
     const { login, logout } = useAuth();
     console.log("offerId, buyerId : ", offerId, buyerId);
     const [inputINR, setInputINR] = useState("");
     const [userid, setUserid] = useState("");
     const [createTrades] = useMutation(createTradeAPI);
     ...
     const [checkMetamaskUser] = useLazyQuery(MetamaskExist);

     useEffect(() => {
          if (!active) {
               login();
               console.log("jsut Login");
               checkMetamaskUser({
                    skip: !account,
                    variables: { metamask: account },
               }).then((res) => {
                    console.log("Metamask exist", account, res);
                    if (
                         res?.data?.isMetamaskExist?.success == false ||
                         !account
                    ) {
                         Swal.fire("Please Register first..!");
                         navigate("/signup");
                    } else {
                         setUserid(res?.data?.isMetamaskExist?.user[0].id);
                    }
               });
          } else {
               checkMetamaskUser({
                    ...
                    console.log("Metamask exist 2", account, res);
                    if (res?.data?.isMetamaskExist?.success == false) {
                         Swal.fire("Please Register first..!");
                         navigate("/signup");
                    } else {
                         setUserid(res?.data?.isMetamaskExist?.user[0].id);
          ...
          <main className="xl:mt-[100px] mt-[80px]">
               <div className="head  w-full h-full bg-client-buyer-img bg-cover text-center py-8">
                    <p className="xl:text-[40px] lg:text-[30px] text-[20px] text-white font-bold">
                         {" "}
                         Buy Bitcoin with Bank Transfer (IDR) - <br />
                                                            ...
                                                            if (active) {
                                                                 createVoucher().then(
                                                                      (res) => {
                                                                                     ...
                                                                                     variables:
                                                                                          {
                                                                                               ...
                                                                                               pay_via: data
                                                                                                    ...
                                                                                                    .offer[0]
                                                                                                    .payment_type[0],
                                                                                               signature: `["${res.seller}", "${res.buyer}", "${res.token}", "${res.offerId}", "${res.tokenName}","${res.amount}", "${res.price}", "${res.signature}"]`,
                                                                                          },
                                                                                }
                                                                           ).then(
                                                                                (
                                                                           ...
                                                                           console.log(
                                                                                "Send to Buyer Chat page",
                                                                                res
                                                                           );
                                                                           navigate(
                                                                                `/offer/buyerchat/${offerId}/${buyerId}`
                                                                           );
                                                                      }
                                                                 );
                                                            } else {
...
export default ClientBuyerPage;
